(() => {
  ...
  var init_constants = __esm({
    "data/constants.js"() {
      ...
      HUMAN_COUNT_INITIAL = 30;
      HUMAN_RADIUS = 0.26;
      HUMAN_LOGICAL_SIZE = 26;
      HUMAN_SPEED_WALK = 0.4;
      HUMAN_SPEED_CLIMB = 0.2;
      ...
      HUMAN_RESCUE_ENABLED = true;
      HUMAN_RESCUE_TIME = 15;
      HUMAN_DROWN_DEATH_SEC = 15;
      HUMAN_FALL_DEATH_HEIGHT = 3;
      HUMAN_DEATH_FADE_SEC = 3;
      HUMAN_DEATH_SKULL_SEC = 2;
      HUMAN_DEATH_REACTION_DIST = 2;
      HUMAN_RENDER_CONFIG = {
        BOUNCE_AMP: 2,
        // 歩行時の跳ね
        SWAY_AMP: 1,
  ...
  function setAllHumans(h) {
    allHumans = h;
  }
  function setNextPair(p) {
    nextPair = p;
  }
  function setSpawnEnabled(e) {
    spawnEnabled = e;
  }
  ...
  var state, elapsed, gravityActive, cascadePhase, cascadeSteps, particles, confetti, floodPhase, floodBlinkStart, floodCountdownEnd, limitWarningUntil, gameoverStart, gameoverCameraY, clearStart, clearCelebrationFired, lastBombCheck, fallingBombs, spaceReleased, humans, allHumans, deadHumans, pair, nextPair, spawnEnabled, rainParticles, thunderFlashTimer, shakeOffset, radarAbstractionMode, cautionAnimTimer, currentPhase, fps, renderSafeLine, isPaused, debugPauseCracking, debugPauseWater, debugMessage, debugMessageTimer, editorHuman, editorState, editorLastSpeech, editorPage;
  ...
  function updateShake(dt2) {
    ...
    if (Math.floor(elapsed / period) > Math.floor((elapsed - dt2) / period)) {
  ...
  __export(collapse_exports, {
    ...
    isBoardStable: () => isBoardStable,
    setupGravity: () => setupGravity,
    startCascade: () => startCascade,
    triggerLimitWarning: () => triggerLimitWarning,
      ...
      for (let x = 0; x < COLS; x++) {
        ...
        if (cell && typeof cell === "object" && cell.renderOffsetY < 0) {
          if (cell.fallDelay > 0) {
            ...
            cell.renderOffsetY += ANIM_FALL_SPEED * dt2;
  ...
  function newPair() {
    ...
    for (let i = 0; i < size; i++) {
      c.push(rand2());
      cracked.push(Math.random() < CRACKED_BLOCK_PROBABILITY);
    }
    return {
      x: 2,
  ...
  function updatePairRender() {
    if (!pair2) return;
    pair2.rx += (pair2.x - pair2.rx) * PAIR_MOVE_LERP;
    pair2.ry += (pair2.y - pair2.ry) * PAIR_FALL_LERP;
    let diff = pair2.rot - pair2.rrot;
    if (diff > 2) diff -= 4;
    if (diff < -2) diff += 4;
    pair2.rrot += diff * PAIR_ROT_LERP;
    if (pair2.rrot < 0) pair2.rrot += 4;
    if (pair2.rrot >= 4) pair2.rrot -= 4;
  }
  function hit(bs) {
    for (let b of bs) {
      ...
      Human = class {
        ...
        think() {
          ...
          if (this.state === "shock") {
            ...
            if (this.timer <= 0) {
              this.state = "walk";
              this.dir = Math.random() < 0.5 ? -1 : 1;
              this.vx = this.dir * HUMAN_SPEED_WALK * SIZE;
              this.hasNoticedCrack = false;
            }
            return;
          }
          if (this.state === "climb_cracked") {
            let gy = Math.floor((this.y + HUMAN_LOGICAL_SIZE / 2) / SIZE);
            let gx2 = Math.round((this.x - OFFSET_X) / SIZE);
            let cell = grid[gy] ? grid[gy][gx2] : null;
            if (!cell || !cell.isCracked) {
              this.state = "fall";
              return;
            }
            let cellTopY = gy * SIZE;
            if (this.y <= cellTopY + SIZE * 0.5) {
              grid[gy][gx2] = null;
              showDebugMessage("RESCUE: CRACKED BLOCK BROKEN");
              this.state = "fall";
              this.vy = 0;
              Promise.resolve().then(() => (init_collapse(), collapse_exports)).then((m) => m.applyLimit());
              return;
            }
            this.vy = -HUMAN_SPEED_CLIMB * SIZE;
            return;
          }
          ...
          let feetY2 = this.y + HUMAN_LOGICAL_SIZE;
          let standingY2 = TOTAL_ROWS * SIZE;
          let wallY = -1;
          ...
          for (let r = 0; r < TOTAL_ROWS; r++) {
            ...
            if (cell) {
              ...
              if (cellY < feetY2 && cellY + SIZE > this.y) {
              ...
              if (cellY >= feetY2 - 2) {
                standingY2 = Math.min(standingY2, cellY);
              }
            }
          }
          if (this.state !== "climb" && feetY2 < standingY2 - 5) {
            if (this.state !== "fall") {
              this.state = "fall";
              this.vx = 0;
              this.say("FALLING");
          ...
          if (this.state === "fall" && feetY2 >= standingY2) {
            this.state = "walk";
            this.say("LANDED");
            this.y = standingY2 - HUMAN_LOGICAL_SIZE;
            this.vy = 0;
            return;
          }
          ...
          if (this.state === "walk" || this.state === "escape") {
            ...
            let nextStandingY = this.getStandingY(nextGx, feetY2);
            if (nextStandingY > feetY2 + 5) {
        ...
        applyVelocity(dt2) {
          ...
          if (this.state === "fall" || this.state === "drown") {
            ...
            gx = Math.max(0, Math.min(COLS - 1, gx));
            let feetY2 = this.y + HUMAN_LOGICAL_SIZE;
            let standingY2 = this.getStandingY(gx, feetY2);
            if (this.vy >= 0 && feetY2 >= standingY2 - 5) {
              if (this.state === "fall") {
                let dist = (this.y - this.fallStartY) / SIZE;
                if (dist >= HUMAN_FALL_DEATH_HEIGHT) {
                  this.die("fall");
                  return;
                }
                this.state = "walk";
          ...
          this.x += this.vx * dt2 * speedMultiplier;
          this.y += this.vy * dt2 * speedMultiplier;
        ...
        update(dt2) {
          this.animTime += dt2;
          this.aiTimer -= dt2;
          if (this.speechTimer > 0) this.speechTimer -= dt2;
          if (this.speechCooldown > 0) this.speechCooldown -= dt2;
          if (this.timer > 0) this.timer -= dt2;
          if (this.state === "goal") {
            this.timer -= dt2;
            if (this.timer <= 0) {
          ...
          this.applyVelocity(dt2);
        }
        onEncounterCrack() {
          this.state = "shock";
          this.timer = HUMAN_SHOCK_SEC;
          this.vx = 0;
          this.vy = 0;
        ...
        die(type) {
          ...
          this.state = "dead";
          this.deathType = type;
          this.deathTimer = HUMAN_DEATH_FADE_SEC;
          this.vx = 0;
          this.vy = 0;
          if (type === "bomb") {
            for (let i = 0; i < 5; i++) {
              let angle = Math.random() * 360 * Math.PI / 180;
              let speed = (3 + Math.random() * 5) * SIZE;
              if (i < 2) {
        ...
        checkWitnessDeath() {
          for (let h of humans) {
            ...
            let dx = h.x - this.x;
            let dy = h.y - this.y;
            let dist = Math.sqrt(dx * dx + dy * dy) / SIZE;
            if (dist < HUMAN_DEATH_REACTION_DIST) {
              h.state = "escape";
              h.dir = dx > 0 ? 1 : -1;
              h.vx = h.dir * HUMAN_SPEED_ESCAPE * SIZE;
              h.say("FEAR");
              h.timer = 2;
            }
          }
        ...
        updateDeath(dt2) {
          if (this.deathTimer > 0) {
            ...
            if (this.deathTimer <= 0) {
              this.skullTimer = HUMAN_DEATH_SKULL_SEC;
            }
  ...
  function updateFloodGrace(dt2) {
    if (floodPhase === "none") return;
    if (floodPhase === "blink") {
      setCautionAnimTimer(Math.max(0, cautionAnimTimer - dt2));
  ...
  function update(dt2) {
    ...
    if (debugMessageTimer > 0) {
      setDebugMessageTimer(debugMessageTimer - dt2);
    }
    updateRain(dt2);
    updateThunder(dt2);
    updateShake(dt2);
    ...
    if (state === "playing" || state === "goal_wait" || state === "gameover" || state === "clear") {
      ...
      for (let h of humans) {
        h.update(dt2 * speedMult);
        if (h.state === "drown" && !h.isDead) {
          h.drownTimer = (h.drownTimer || 0) + dt2 * speedMult;
          if (h.drownTimer >= HUMAN_DROWN_DEATH_SEC) {
            h.die("drown");
          }
        }
        if (h.y <= (cameraY + ROWS + 10) * SIZE) {
          remaining.push(h);
        }
      }
      setHumans(remaining);
    ...
    if (state === "gameover") {
      ...
      if (elapsed >= WATER_START_DELAY) {
        setWater(water + dt2 * WATER_SPEED * GAMEOVER_WATER_MULT);
    ...
    if (isCautionPause()) {
      updateFloodGrace(dt2);
      return;
    }
    if (!debugPauseWater && elapsed >= WATER_START_DELAY) {
      setWater(water + dt2 * WATER_SPEED);
    }
    updateBombs(dt2);
    ...
    updateFloodGrace(dt2);
    if (state !== "playing") return;
    if (pair2) updatePairRender();
    if (gravityActive) updateCascade(dt2);
    updateCrackingBlocks(dt2);
    if (!nextPair && spawnEnabled) setNextPair(newPair());
    if (!pair2 && spawnEnabled && nextPair) {
      setPair(nextPair);
      setNextPair(newPair());
    }
    addDrop(dt2);
  ...
  function updateBombs(dt2) {
    ...
    for (let i = fallingBombs.length - 1; i >= 0; i--) {
      ...
      let nextY = b.y + BOMB_FALL_SPEED * dt2;
      let visualTop = TOTAL_ROWS;
      for (let y = 0; y < TOTAL_ROWS; y++) {
        let cell = grid[y][b.x];
        if (cell) {
  ...
  function updateCrackingBlocks(dt2) {
  ...
  function checkRowFullCracked(y) {
    if (y < 0 || y >= TOTAL_ROWS) return false;
    for (let x = 0; x < COLS; x++) {
      let cell = grid[y][x];
  ...
  function updateRescue(dt2) {
    if (!HUMAN_RESCUE_ENABLED || state !== "playing") return;
    for (let y = Math.floor(renderSafeLine); y < TOTAL_ROWS; y++) {
      if (checkRowFullCracked(y)) {
        let humansAtRow = humans.filter((h) => {
          let gy = Math.floor((h.y + HUMAN_LOGICAL_SIZE - 2) / SIZE);
          return gy === y + 1;
        });
        if (humansAtRow.length > 0) {
          rowFullTimers[y] = (rowFullTimers[y] || 0) + dt2;
          if (rowFullTimers[y] >= HUMAN_RESCUE_TIME) {
            let victim = humansAtRow[0];
            if (victim.state !== "climb_cracked" && victim.state !== "shock") {
              victim.state = "climb_cracked";
              showDebugMessage(`RESCUE: Floor ${y} opened by Human ${victim.id}`);
              rowFullTimers[y] = 0;
            }
          }
        } else {
  ...
  function checkBombDamage(bx, by) {
    const worldX = bx * SIZE + OFFSET_X + SIZE / 2;
    const worldY = by * SIZE + SIZE / 2;
    const radius = SIZE * 1.5;
    for (let h of humans) {
      if (h.isDead) continue;
      let dx = h.x - worldX;
      let dy = h.y + HUMAN_LOGICAL_SIZE / 2 - worldY;
  ...
  function drawHumans(ctx2, screenYOffset, humansList = null) {
    ...
    for (let h of sortedHumans) {
      ...
      if (h.isDead && h.deathTimer > 0) {
        alpha = Math.max(0, h.deathTimer / HUMAN_DEATH_FADE_SEC);
      }
      ...
      if (h.state === "walk" || h.state === "escape" || h.state === "lost") {
        ...
        drawPart(ctx2, P_HAND, -bodyR - cfg.HAND_X_OFF, bodyR * cfg.HAND_Y_OFF_B, cfg.HAND_SIZE, cfg.HAND_SIZE);
        drawPart(ctx2, P_HAND, bodyR - cfg.HAND_X_OFF / 2, bodyR * cfg.HAND_Y_OFF_B, cfg.HAND_SIZE, cfg.HAND_SIZE);
        drawPart(ctx2, P_BODY, -bodyR, -bodyR * 2, bodyR * 2 * (cfg.BODY_SIZE_RATIO / 2), bodyR * 2 * (cfg.BODY_SIZE_RATIO / 2));
        drawPart(ctx2, P_FOOT_N, -bodyR / 2 - cfg.FOOT_X_OFF, cfg.FOOT_Y_OFF, cfg.FOOT_W, cfg.FOOT_H);
        drawPart(ctx2, P_FOOT_N, bodyR / 2 - cfg.FOOT_X_OFF, cfg.FOOT_Y_OFF, cfg.FOOT_W, cfg.FOOT_H);
        let s = speechSprites["\u03A3"];
        if (s) {
          ctx2.drawImage(humanAtlas, s.x, s.y, s.w, s.h, -s.w / 2, -bodyR * 4, s.w, s.h);
        }
      } else if (h.isDead) {
        if (h.deathTimer > 0) {
          if (h.deathType === "bomb") {
            for (let p of h.deathParts) {
              ...
              ctx2.rotate(p.rot);
              drawPart(ctx2, P_BODY, -bodyR / 2, -bodyR / 2, bodyR, bodyR);
              ctx2.restore();
            }
          } else if (h.deathType === "drown") {
            ctx2.filter = "brightness(0.3)";
            drawPart(ctx2, P_BODY, -bodyR, -bodyR * 2, bodyR * 2 * (cfg.BODY_SIZE_RATIO / 2), bodyR * 2 * (cfg.BODY_SIZE_RATIO / 2));
            ctx2.filter = "none";
          } else if (h.deathType === "fall") {
            let squash = Math.max(0.1, h.deathTimer / HUMAN_DEATH_FADE_SEC);
            ctx2.scale(1.5, squash);
            drawPart(ctx2, P_BODY, -bodyR, -bodyR * 2, bodyR * 2 * (cfg.BODY_SIZE_RATIO / 2), bodyR * 2 * (cfg.BODY_SIZE_RATIO / 2));
            if (h.deathTimer > HUMAN_DEATH_FADE_SEC * 0.7) {
              ctx2.fillStyle = "rgba(200,200,200,0.5)";
              for (let i = 0; i < 3; i++) {
                ctx2.beginPath();
                ctx2.arc((i - 1) * 10, -10, 8, 0, Math.PI * 2);
                ctx2.fill();
              }
            }
          }
        ...
        if (h.skullTimer > 0) {
          let sAlpha = h.skullTimer / HUMAN_DEATH_SKULL_SEC;
          let sSize = (1 + (1 - sAlpha)) * SIZE;
          let sY = -(1 - sAlpha) * SIZE * 3;
          ctx2.globalAlpha = sAlpha;
          ctx2.font = `bold ${sSize}px sans-serif`;
          ctx2.textAlign = "center";
          ctx2.fillText("\u{1F480}", 0, sY);
        }
  ...
  function updateEditor(dt2) {
    if (!editorHuman) return;
    editorHuman.animTime += dt2;
    if (editorHuman.speechTimer > 0) editorHuman.speechTimer -= dt2;
  ...
  function drawRadar() {
    ...
    deadHumans.forEach((h) => {
      let hm = (TOTAL_ROWS - h.y / SIZE - 1) * METERS_PER_ROW;
      let ry = radarYByMeters(hm);
      let rx = rx0 + (h.x - OFFSET_X) / (COLS * SIZE) * RADAR_INNER_W;
  ...
  function drawSurvivalList() {
    ...
    ctx.fillStyle = "#fff";
    allHumans.forEach((h, i) => {
      let y = startY + i * spacing;
      if (h.isDead && h.skullTimer <= 0) {
        ctx.fillStyle = "#444";
        ctx.fillText("\u25CF", startX, y);
        ctx.font = "10px sans-serif";
        ctx.fillText("\u{1F480}", startX + 12, y);
      } else {
  ...
  function drawNextBlock() {
    if (!nextPair) return;
    const boxX = 10, boxY = 10, boxW = 80, boxH = 40;
    ctx.save();
    ...
    ctx.fillText("NEXT", boxX + 5, boxY + 12);
    const miniSize = 12;
    const centerX = boxX + boxW / 2;
    const centerY = boxY + boxH / 2 + 5;
    const b1 = { x: 0, y: 0, c: nextPair.c1 };
    const b2 = { x: 0, y: 0, c: nextPair.c2 };
    if (nextPair.dir === 0) b2.y = 1;
    else b2.x = 1;
    [b1, b2].forEach((b) => {
      ctx.fillStyle = COLORS[b.c];
      ctx.fillRect(centerX + (b.x - 0.5) * miniSize, centerY + (b.y - 0.5) * miniSize, miniSize, miniSize);
      ctx.strokeStyle = "#000";
      ctx.lineWidth = 1;
      ctx.strokeRect(centerX + (b.x - 0.5) * miniSize, centerY + (b.y - 0.5) * miniSize, miniSize, miniSize);
    });
    ctx.restore();
  }
  function draw(dt2 = 0) {
    ...
    if (state === "title" || state === "intro") {
      ...
      drawGridBlocks();
      if (state === "playing" && pair2) {
        renderBlocks(pair2).forEach((b) => {
          drawBlockCell(b.x * SIZE + OFFSET_X, screenY(b.y), COLORS[b.c], null, b.isCracked);
        });
      }
  ...
  function drawBlockCell(px, py, color, timer = null, isCracked = false) {
    ctx.fillStyle = color;
    ...
    if (timer !== null && timer <= ANIM_CRACK_SEC) {
      drawCrack(px, py, 1 - timer / ANIM_CRACK_SEC);
    } else if (isCracked) {
      drawCrack(px, py, 1);
    }
  }
  ...
  function setupInput() {
    ...
    document.addEventListener("keydown", (e) => {
      ...
      if (e.key === "6") {
        const alive = humans.filter((h) => !h.isDead);
        if (alive.length > 0) {
          alive[Math.floor(Math.random() * alive.length)].die("none");
          showDebugMessage("DEBUG: Random Human Died");
        }
      }
      if (e.key === "7") {
        setSpawnEnabled(!spawnEnabled);
        showDebugMessage("SPAWN: " + (spawnEnabled ? "ON" : "OFF"));
      }
      ...
      if (e.key === "ArrowUp") {
        const prevRot = pair2.rot;
        const prevX = pair2.x;
        pair2.rot = (pair2.rot + 1) % 4;
        if (hit(blocks(pair2))) {
          const kicks = [-1, 1, -2, 2];
          let ok = false;
