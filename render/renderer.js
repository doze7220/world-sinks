...
import { TEXT_JP as TEXT } from '../text/human_text_jp.js';
import { pair, renderBlocks } from '../entities/block.js';
...
export function drawWater() {
  ...
  if (STATE.currentPhase >= 3) amp = 10;
  if (STATE.currentPhase === 4) amp = 10 * (1 + (Math.random() - 0.5) * 0.4);
  ...
  if (STATE.state === "playing" || STATE.state === "gameover") {
    ...
    if (STATE.state !== "gameover") {
      ...
      ctx.strokeText(WORLD.waterMeters() + "m", 18, waterBaseY - 2);
      ctx.fillText(WORLD.waterMeters() + "m", 18, waterBaseY - 2);
    }
  }
  ctx.restore();
}
...
export function drawCautionUI() {
  ...
  ctx.strokeStyle = CONST.UI_CAUTION_STOKE_COLOR;
  ctx.strokeText(CONST.UI_CAUTION_TEXT, 0, 0);

  ctx.fillStyle = CONST.UI_CAUTION_COLOR;
  ctx.fillText(CONST.UI_CAUTION_TEXT, 0, 0);
...
export function drawRadar() {
  ...
  const drawLine = (m, color, label) => {
    ...
    rctx.beginPath(); rctx.moveTo(rx0, ry); rctx.lineTo(rx0 + CONST.RADAR_INNER_W + 40, ry); rctx.stroke();
    rctx.fillStyle = color; rctx.font = "11px sans-serif"; rctx.textAlign = "right";
    rctx.fillText(`${label} ${Math.round(m)}m`, rx0 + CONST.RADAR_INNER_W + 55, ry - 5);
  };
...
function drawNextBlock() {
  ...
  // STATE.nextPair is { x, y, c1, c2, dir }
  const b1 = { x: 0, y: 0, c: STATE.nextPair.c1 };
  const b2 = { x: 0, y: 0, c: STATE.nextPair.c2 };
  if (STATE.nextPair.dir === 0) b2.y = 1;
  else b2.x = 1;

  [b1, b2].forEach(b => {
    ctx.fillStyle = CONST.COLORS[b.c];
...
function drawPauseUI() {
  ctx.save();
  ctx.fillStyle = CONST.UI_PAUSE_OVERLAY_COLOR;
  ctx.fillRect(0, 0, CONST.TOTAL_W, CONST.MAIN_H);

  // 点滅ロジック
  const now = performance.now() / 1000;
  if ((now % CONST.UI_PAUSE_BLINK_PERIOD) < (CONST.UI_PAUSE_BLINK_PERIOD / 2)) {
    ctx.fillStyle = CONST.UI_PAUSE_TEXT_COLOR;
    ctx.font = CONST.UI_PAUSE_FONT;
    ctx.textAlign = "center";
    ctx.textBaseline = "middle";
    ctx.fillText(CONST.UI_PAUSE_TEXT, CONST.TOTAL_W / 2, CONST.MAIN_H / 2);
  }
  ctx.restore();
}

function drawGameGuides() {
  ctx.strokeStyle = CONST.UI_SIDE_LINE_COLOR; 
  ...
  ctx.beginPath(); ctx.moveTo(CONST.OFFSET_X - 60, gy); ctx.lineTo(CONST.OFFSET_X + CONST.COLS * CONST.SIZE, gy); ctx.stroke();
  drawGuideText("ゴール", CONST.GOAL_METERS + "m", 15, gy, CONST.GOAL_LINE);
  ...
  drawGuideText("安全高度", currentM + "m", 10, sly, CONST.SAFE_LINE);
}
...
function drawLimitWarning() {
  ...
  if (STATE.floodPhase === "none" && STATE.elapsed < STATE.limitWarningUntil) {
    ...
    const lh = 22;
    const totalH = (CONST.LIMIT_WARNING_LINES.length - 1) * lh;
    CONST.LIMIT_WARNING_LINES.forEach((line, i) => {
...
function drawGridBlocks() {
  for (let y = 0; y < CONST.ROWS + 1; y++) {
    for (let x = 0; x < CONST.COLS; x++) {
      ...
      if (gy >= 0 && gy < CONST.TOTAL_ROWS && WORLD.grid[gy][x] && WORLD.grid[gy][x] !== 9) {
...
export function drawEditMode() {
  ...
  EDITOR.EDITOR_BUTTONS.forEach(btn => {
    if (btn.type === 'page_nav') {
      ...
      if (btn.dir === 1) {
        ctx.textAlign = "center";
        ctx.fillText(EDITOR.PAGES[STATE.editorPage].name, (btn.rect.x + 10 - 100), btn.rect.y + 18);
        ctx.textAlign = "left";
      }
  ...
  ctx.fillText("Press SPACE to return Title", 10, CONST.MAIN_H - 10);
}
...
export function drawCrack(px, py, progress) {
  ...
  ctx.beginPath(); ctx.moveTo(px + CONST.SIZE * 0.2, py); ctx.lineTo(px + CONST.SIZE * 0.4, py + CONST.SIZE * 0.4);
  ctx.lineTo(px + CONST.SIZE * 0.3, py + CONST.SIZE * 0.7); ctx.lineTo(px + CONST.SIZE * 0.6, py + CONST.SIZE);
  ctx.moveTo(px + CONST.SIZE * 0.4, py + CONST.SIZE * 0.4); ctx.lineTo(px + CONST.SIZE * 0.8, py + CONST.SIZE * 0.6);
