  ...
  if (STATE.state === "title" || STATE.state === "intro") {
    ...
    if (STATE.state === "playing" && pair) {
      renderBlocks(pair).forEach(b => {
        drawBlockCell(b.x * CONST.SIZE + CONST.OFFSET_X, screenY(b.y), CONST.COLORS[b.c]);
      });
    }
...
export function drawBlockCell(px, py, color, timer = null) {
  ...
  if (timer !== null && timer <= CONST.ANIM_CRACK_SEC) drawCrack(px, py, 1.0 - (timer / CONST.ANIM_CRACK_SEC));
}
